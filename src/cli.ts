...
import { loadFsm } from "./fsm/engine.js";
import { existsSync } from "fs";
import { join as pathJoin } from "path";

const LEVELS = ["S", "M", "L", "CRITICAL"] as const;
...
const commands: Record<string, Command> = {
  ...
  task: new Command("task", "Create a new task")
    ...
    .action(async (args: string[], opts: CommandOpts) => {
  ...
  gate: new Command("gate", "Run quality gates")
    ...
    .action(async (args: string[], opts: CommandOpts) => {
      const cwd = process.cwd();
      const harnessDir = resolveHarnessDir(cwd);
      const profileName = (opts.profile as string) || "standard";
      ...
      const taskId = (opts.task as string) || "cli-run";
...
function printTaskStatus(task: ReturnType<typeof loadTask>): void {
  console.log(`\nTask: ${task!.id}`);
  console.log(`  Title: ${task!.title}`);
  console.log(`  Level: ${task!.level}`);
  console.log(`  State: ${task!.stateMachine.current}`);
...
function resolveHarnessDir(cwd: string): string {
  const candidates = [
