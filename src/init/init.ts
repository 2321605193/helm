...
import type { InstallOptions, DetectedRuntime, ManifestEntry, ScanResult } from "./types.js";
...
export async function runInit(cwd: string, opts: InstallOptions = {}): Promise<void> {
  ...
  if (opts.dryRun) {
    ...
    for (const f of diff.newFiles) console.log(`  + create   ${f}`);
    for (const f of diff.updatedFiles) console.log(`  ~ update   ${f}`);
...
function buildManifest(cwd: string, scan: ScanResult, skipped: Set<string>): ManifestEntry[] {
  ...
  for (const [destPath] of TEMPLATE_FILES) {
    ...
    if (skipped.has(destPath)) {
      ...
      if (baseline) manifest.push({ path: destPath, hash: baseline });
      continue;
    }
...
function getTemplateDir(): string {
  ...
  for (const c of candidates) {
    if (existsSync(join(c, "base", "AGENTS.md"))) return c;
  }
  return candidates[0];
}
...
function generateRuntimeSkill(cwd: string, runtime: DetectedRuntime): void {
  const skillContent = `---
name: helm
...
`;
  ...
  const skillPaths: Record<DetectedRuntime, string> = {
