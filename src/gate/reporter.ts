...
import type { GateRunReport, GateResult } from "./types.js";
...
export function buildGateReportMarkdown(report: GateRunReport): string {
  const lines: string[] = [
  ...
  if (failures.length > 0) {
    ...
    for (const r of failures) {
      ...
      if (r.stderr) lines.push("```", r.stderr.trim(), "```", "");
    }
  }
