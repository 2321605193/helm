...
export function createTask(title, harnessDir, opts = {}) {
...
export function listTasks(cwd) {
    ...
    if (!existsSync(tasksDir))
        return [];
    const entries = readdirSync(tasksDir).filter((e) => {
        const full = join(tasksDir, e);
        return existsSync(join(full, "task.json"));
    });
    return entries
        .map((e) => loadTask(join(tasksDir, e)))
        .filter(Boolean);
}
function profileNameForLevel(level) {
    switch (level) {
        case "S":
            return "minimal";
        case "M":
